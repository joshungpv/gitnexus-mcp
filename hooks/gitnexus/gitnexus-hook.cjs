...
// Helper to log errors safely to a dedicated file
function logError(err, context = '') {
  try {
    ...
    const logPath = path.join(logDir, 'gitnexus-hook.log');
...
function findCanonicalRepoRoot(cwd) {
  try {
    ...
    logError(err, 'findCanonicalRepoRoot');
    return null;
  }
}
...
function runGitNexusCli(cliPath, args, cwd, timeout) {
  const { spawnSync } = require('child_process');
  const dbLockProbe = require('./hook-db-lock-probe.cjs');
  ...
  const guard =
    isWin || typeof dbLockProbe.resolveUnixGuardTimeout !== 'function' ? null : dbLockProbe.resolveUnixGuardTimeout();
...
function handlePreToolUse(input) {
  ...
  let release;
  try {
    release = acquireHookSlot(gitNexusDir);
  } catch (err) {
    logError(err, 'PreToolUse (acquireHookSlot)');
  }
  if (!release) {
  ...
  try {
    ...
    logError(err, 'PreToolUse (cli run)');
  } finally {
    if (release) release();
  }
...
function handlePostToolUse(input) {
  ...
  try {
    ...
    logError(err, 'PostToolUse (git rev-parse)');
    return;
  }
  ...
  const { formatAnalyzeCommand, buildRunnerArgv, resolveInvocationMode } = require('./resolve-analyze-cmd.cjs');
  ...
  try {
    ...
    const { program, args: runnerArgs } = buildRunnerArgv(mode, args);

    // Redirect bg outputs to background log in `.gitnexus` folder
    const logFile = path.join(gitNexusDir, 'reindex-bg.log');
    let outFd = 'ignore';
    let errFd = 'ignore';
    try {
      outFd = fs.openSync(logFile, 'a');
      errFd = outFd;
      fs.writeSync(outFd, `\n[${new Date().toISOString()}] Kích hoạt reindex tự động chạy ngầm...\n`);
    } catch (logErr) {
      logError(logErr, 'PostToolUse (bg log openSync)');
    }

    const bgProcess = spawn(program, runnerArgs, {
      cwd,
      detached: true,
    ...
    sendHookResponse(
      'PostToolUse',
      `GitNexus index is stale. Đã tự động kích hoạt reindex chạy ngầm (Log: .gitnexus/reindex-bg.log).`,
    );
  } catch (err) {
    logError(err, 'PostToolUse (trigger bg index)');
    sendHookResponse(
      'PostToolUse',
      `GitNexus index is stale. Lỗi kích hoạt reindex chạy ngầm. Vui lòng chạy: \`${analyzeCmd}\``,
    );
  }
}
